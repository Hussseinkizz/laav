...
const BottomBar = (props: Props) => {
  ...
  return (
    <nav className="w-full shadow-lg _bg-theme py-2 px-4 flex gap-4 items-center justify-between z-10">
      <Link href="/feeds" passHref className="flex">
        <Icons.HiUsers className="w-6 h-6 _text-theme--hoverable" />
      </Link>
      <Link href="/discover" passHref className="flex">
        <Icons.HiGift className="w-6 h-6 _text-theme--hoverable" />
      </Link>
      <Link
        href="/"
        passHref
      ...
      <Link href="/likes" passHref className="flex">
        <IoSparklesSharp className="w-6 h-6 _text-theme--hoverable" />
      </Link>
    </nav>
  );
};
